...
void write_subhg(std::string dataset, std::string alg, std::map<size_t, strvec > e_id_to_edge, std::set<size_t> subhg){
    ...
    std::cout<<file<<"\n";
    std::string efile = "../output/density/"+dataset+"_"+alg+"_e.csv";
    std::cout<<efile<<"\n";
    std::stringstream ss;
...
void clique_densest_hg(std::string dataset,std::map<size_t, strvec > e_id_to_edge, std::map<std::string, std::set<size_t> > inc_dict, strvec init_nodes, std::set<size_t> &subhg ){
    ...
    size_t sum_deg = 0, num_nodes = init_nodes.size(), min_deg = e_id_to_edge.size();
    double density = 0; 
    std::cout<<"N = "<<num_nodes<<" |E| = "<<min_deg<<"\n";
    ...
    density = double(sum_deg)/(2*num_nodes);
    ...
    while(!initnodes.empty()){
        ...
        double d2 = double(sum_deg)/(2*num_nodes);
        // std::cout<<d2<<"\n";
        if(d2>density){
            density = d2;
            for(auto e_id:affected_edges){
    ...
    // std::cout<<subhg.size()<<"\n";
    std::cout<<"Maximum clique graph density "<<density<<"\n";
    double density_nbr = nbr_density(subhg,e_id_to_edge);
    std::cout<<"Corresponding neighbourhood based density "<<density_nbr<<"\n";
    write_result(dataset,"clique",density_nbr,density);
...
int main(int argc, char *argv[])
{
    ...
    if(alg=="clique"){
        ...
        h.init_nodes.clear();
        h.initialise();
        clique_densest_hg(h.dataset,h.e_id_to_edge,h.inc_dict,h.init_nodes,subhg);
        write_subhg(h.dataset,alg,h.e_id_to_edge,subhg);
    }
